...
const INITIAL_SCHEMA_SQL = `-- Tenants table
...
export async function runMigrations() {
  try {
    ...
    let rawSql = INITIAL_SCHEMA_SQL;

    // Try reading file if available, otherwise use embedded schema
    const distPath = path.join(__dirname, 'migrations', '001_initial_schema.sql');
    const srcPath = path.join(__dirname, '..', '..', 'src', 'db', 'migrations', '001_initial_schema.sql');

    if (fs.existsSync(distPath)) {
      rawSql = fs.readFileSync(distPath, 'utf8');
    } else if (fs.existsSync(srcPath)) {
      rawSql = fs.readFileSync(srcPath, 'utf8');
    }
